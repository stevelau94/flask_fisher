...
def reject_drift(did):
    drift = Drift.query.filter(
        Drift.id == did, Drift.gifter_id == current_user.id, Drift._pending == PendingStatus.Waiting.value,
...
def mailed_drift(did):
    with db.auto_commit():
        drift = Drift.query.filter(Drift.id == did, Drift.gifter_id == current_user.id,
                                   Drift._pending == PendingStatus.Waiting.value, Drift.status == 1).first_or_404()
        # 查找索要者
        ...
        wish = Wish.query.filter(Wish.isbn == drift.isbn, Wish.uid == drift.requester_id, Wish.launched == False,
